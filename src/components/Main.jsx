      ...
      <div className="card">
        ...
        <div className="card--country">
          ...
          >
            ...
            <a className= 'card--google--map--link' href={props.googleMapsUrl} target="_blank" rel="noopener noreferrer">
              View on Google Maps
            </a>
          </div>
          <h3 className="card--location--name">{props.title}</h3>
          <h4 className="card--date">{props.startDate}, {props.endDate}</h4>
          <p className="card--location--discription">{props.description}</p>
        </div>
      </div>
      {props.location === 'Norway'? null: <hr class="card__line"></hr>}
